...
{
	private static readonly ParallelOptions _parallelOptionsTaskScheduler = new ParallelOptions { MaxDegreeOfParallelism = App.MaxDegreeOfParallelism(), TaskScheduler = TaskScheduler.Current };
	...
	/// <summary>
	/// Processes the collection in parallel using the specified action with default merge options.
	/// </summary>
	/// <typeparam name="T">The type of the elements in the collection.</typeparam>
	/// <param name="action">The action to apply to each item in the collection.</param>
	/// <param name="collection">The collection to process.</param>
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	[Pure]
	private static void ProcessCollectionWithAsParallelWithMergeOptionsDefault<T>(Action<T> action, IEnumerable<T> collection)
	{
	...
	/// <summary>
	/// Processes the collection using the specified action with a load-balanced partitioner for efficient parallel iteration.
	/// </summary>
	/// <typeparam name="T">The type of the elements in the collection.</typeparam>
	/// <param name="action">The action to apply to each item in the collection.</param>
	/// <param name="collection">The collection to process.</param>
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	[Pure]
	private static void ProcessCollectionWithForEachPartitionerLoadBallanced<T>(Action<T> action, IEnumerable<T> collection)
	{
		var list = collection as List<T> ?? collection.ToList();
		var rangePartitioner = Partitioner.Create(list, loadBalance: true);

		_ = Parallel.ForEach(rangePartitioner, person => action(person));
	}

	/// <summary>
	/// Processes the collection using the specified action with Parallel.For for efficient parallel iteration.
	/// </summary>
	/// <typeparam name="T">The type of the elements in the collection.</typeparam>
	/// <param name="action">The action to apply to each item in the collection.</param>
	/// <param name="collection">The collection to process.</param>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	[Pure]
	private static void ProcessCollectionWithParallelFor<T>(Action<T> action, IEnumerable<T> collection)
	{
		var array = collection as T[] ?? collection.ToArray();
		_ = Parallel.For(0, array.Length, (index) => action(array[index]));
	}

	/// <summary>
	/// Processes the collection using the specified action with Parallel.For and a custom TaskScheduler for efficient parallel iteration.
	/// </summary>
	/// <typeparam name="T">The type of the elements in the collection.</typeparam>
	/// <param name="action">The action to apply to each item in the collection.</param>
	/// <param name="collection">The collection to process.</param>
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	[Pure]
	private static void ProcessCollectionWithParallelForMaxDegreeOfParallelismTaskScheduler<T>(Action<T> action, IEnumerable<T> collection)
	{
		var array = collection as T[] ?? collection.ToArray();
		_ = Parallel.For(0, array.Length, _parallelOptionsTaskScheduler, (index) => action(array[index]));
	}
	...
	public static void FastProcessor<T>(this IEnumerable<T> collection, Action<T> action)
	{
		...
		static void ProcessCollectionWithValueType(Action<T> action, IEnumerable<T> processedCollection, int size)
		{
			...
			{
				ProcessCollectionWithParallelFor(action, processedCollection);
			}
			else if (size >= 512)
			{
				ProcessCollectionWithAsParallelWithMergeOptionsNotBuffered(action, processedCollection);
		...
		static void ProcessCollectionWithReferenceType(Action<T> action, IEnumerable<T> processedCollection, int size)
		{
			if (size >= 4096)
			{
				ProcessCollectionWithParallelForMaxDegreeOfParallelismTaskScheduler(action, processedCollection);
			}
			else if (size >= 512)
			{
				ProcessCollectionWithParallelFor(action, processedCollection);
			...
			{
				ProcessCollectionWithAsParallelWithMergeOptionsDefault(action, processedCollection);
			}
			else if (size >= 128)
			{
				ProcessCollectionWithAsParallelWithMergeOptionsAutoBuffered(action, processedCollection);
			...
			{
				ProcessCollectionWithForEachPartitionerLoadBallanced(action, processedCollection);
			}
		}
	}
}
