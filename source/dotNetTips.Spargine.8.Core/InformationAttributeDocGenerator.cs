...
public static class InformationAttributeDocGenerator
{
	...
	private static void GenerateDocument(Type type, StringBuilder sb)
	{
		...
		_ = sb.AppendLine(CultureInfo.CurrentCulture, $"## {type.Name}");
		_ = sb.AppendLine();
	...
	private static string GetMemberSignature(MemberInfo member)
	{
		switch (member.MemberType)
		{
			case MemberTypes.Constructor:
				...
				var constructorName = constructor.DeclaringType.Name; // Use the class name for the constructor
				var constructorParameters = constructor.GetParameters();
				var constructorParameterList = string.Join(", ", Array.ConvertAll(constructorParameters, p => $"{p.ParameterType.Name} {p.Name}"));
			...
			case MemberTypes.Method:
				...
				var parameterList = string.Join(", ", Array.ConvertAll(parameters, p => $"{p.ParameterType.Name} {p.Name}"));
				return $"{returnType} {methodName}({parameterList})";

			case MemberTypes.Property:
				var property = (PropertyInfo)member;
				return $"{property.PropertyType.Name} {property.Name} {{ get; set; }}";
	...
	[Information(nameof(GenerateMarkdownDocumentForAssembly), "David McCarter", "11/18/2024", BenchMarkStatus = BenchMarkStatus.None, UnitTestStatus = UnitTestStatus.Completed, OptimizationStatus = OptimizationStatus.Completed, Status = Status.New)]
	public static string GenerateMarkdownDocumentForAssembly(Assembly assembly, string fileName)
	{
		var result = GenerateMarkdownDocumentForAssembly(assembly);

		// Use or create filename
		fileName = fileName.HasValue() ? fileName : $"{assembly.GetName().Name}.md";
